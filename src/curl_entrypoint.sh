...
if [[ $status_code_movie -ne 200 ]] ; then
  curl -XPUT -H 'Content-Type:application/json' --data-binary "@/home/curl_user/schemas_film.json" "${ELASTIC_HOST}":"${ELASTIC_PORT}"/"${ELASTIC_INDEX_FILM}"
else echo "scheme ${ELASTIC_INDEX_FILM} exists"
fi
if [[ $status_code_genre -ne 200 ]] ; then
  curl -XPUT -H 'Content-Type:application/json' --data-binary "@/home/curl_user/schemas_genre.json" "${ELASTIC_HOST}":"${ELASTIC_PORT}"/"${ELASTIC_INDEX_GENRE}"
else echo "scheme ${ELASTIC_INDEX_GENRE} exists"
fi
if [[ $status_code_person -ne 200 ]] ; then
